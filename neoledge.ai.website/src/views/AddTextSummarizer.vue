...
function addTextSummarizer() {

    if (!textSummarizer.value.name) {
        nameError.value = true;
        return;
    }

    if (!textSummarizer.value.language) {
        languageError.value = true;
        return;
    }

    if (!textSummarizer.value.url) {
        urlError.value = true;
        return;
    }
    console.log(textSummarizer.value);
    // If all fields are filled, proceed with resumeText logic
    axiosInstance.post('api/textProcessor', textSummarizer.value).then(() => {
        resetData();
        emit("confirm");
    });
}
...
function resetData() {
    textSummarizer.value.name = null;
    textSummarizer.value.language = null;
    textSummarizer.value.url = null;
...
<template>
    <div class="card flex justify-content-center" v-if="show">
        <Dialog v-model:visible="show" modal header="Text Summarizer Tool" :style="{ width: '30vw' }">
            <h5>Name</h5>
            <InputText v-model="textSummarizer.name" type="text" :required="true" placeholder="Name of Text Summarizer"
            ...
            <br>
            <h5>Language</h5>
            <Dropdown v-model="textSummarizer.language" :options="dropdownLanguages"
            ...
            <h5>Url</h5>
            <InputText v-model="textSummarizer.url" type="text" :required="true" placeholder="Url of Data Preprocessor"
