...
const props = defineProps(['idText', 'visible']);
...
function deleteText() {
    axiosInstance.delete('api/text/' + props.idText).then(() => {
        emit("confirm");
    });
...
<template>
    <div class="card flex justify-content-center" v-if="show">
        <Dialog v-model:visible="show" modal header="Delete Text ?" :style="{ width: '30vw' }">
            <h6>
